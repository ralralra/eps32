...
| 외부 LED (긴 다리 +) | **IO16** | 아래쪽 디지털 헤더 |
| 외부 LED (짧은 다리 −) | 저항(220Ω쯤) 거쳐 **GND** | 아무 GND나 |
...
| 외부 LED (긴 다리 +) | **GPIO 16** | 내장 LED와 똑같이 켜지고 꺼짐 — 멀리서도 보이게 |
| 외부 LED (짧은 다리 −) | 저항(220Ω쯤) 거쳐 GND | 저항 없이 직결하면 LED가 일찍 죽을 수 있음 |

LED 동작: 대기 중 꺼짐 → 출석 세션 **천천히 깜빡**(0.5초) → 등록 세션 **빠르게 깜빡**(0.15초) → 태그 처리 중 켜짐.
(내장 LED와 외부 LED는 항상 같이 동작합니다. 외부 LED를 안 달아도 그대로 동작해요.)
