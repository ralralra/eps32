...
D1 R32는 보드에 인쇄된 라벨(D10, A1 …)과 GPIO 번호가 다릅니다.
아래 표의 **보드 라벨 위치에 그대로 꽂으면** 코드는 수정 없이 동일하게 동작합니다.
(같은 GPIO — 우노 배치의 SPI 핀이 D10~D13이라서 번호가 딱 맞아떨어집니다)

| RC522 핀 | D1 R32 보드 라벨 | 실제 GPIO | 설명 |
|---|---|:---:|---|
| **3.3V** | **3.3V** (전원 헤더) | — | 전원 (5V 절대 금지!) |
| **RST** | **D6** | 27 | 리셋 |
| **GND** | **GND** | — | 접지 |
| **IRQ** | (연결 안 함) | — | 사용하지 않음 |
| **MISO** | **D12** | 19 | SPI 데이터 (모듈→보드) |
| **MOSI** | **D11** | 23 | SPI 데이터 (보드→모듈) |
| **SCK** | **D13** | 18 | SPI 클럭 |
| **SDA(SS)** | **D10** | 5 | SPI 선택 |

추가 부품 (선택):

| 부품 | D1 R32 보드 라벨 | 실제 GPIO |
|---|---|:---:|
| 수동부저 (+) | **A1** | 4 |
| 수동부저 (−) | GND | — |
| 상태 LED | 보드 내장 파란 LED | 2 |

> 보드 라벨↔GPIO 전체 대응표: [`../../esp32-class-projects/01_docs/pinmap_wemos_d1_r32.md`](../../esp32-class-projects/01_docs/pinmap_wemos_d1_r32.md)
